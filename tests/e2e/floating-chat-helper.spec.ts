...
test.describe('Floating Chat Helper E2E', () => {
  ...
  test.describe('Error Handling', () => {
    test('should display error message when API fails', async ({ page }) => {
      ...
      // Check for the chat panel's local error message
      const chat = page.getByRole('dialog', { name: 'Dentabot' });
      await expect(
        chat.getByText(
    ...
    test('should clear error when new message is sent', async ({ page }) => {
      ...
      await page.route('**/api/genai/chatbot/help', async (route) => {
        ...
        if (callCount === 1) {
          ...
          const response =
            `data: {"choices":[{"delta":{"content":"Success on retry"}}]}\n\ndata: [DONE]\n\n`;
          await route.fulfill({
            status: 200,
            headers: { 'content-type': 'text/event-stream' },
            body: response,
      ...
      // Wait for the chat panel's local error
      const chat = page.getByRole('dialog', { name: 'Dentabot' });
      await expect(chat.getByRole('alert')).toBeVisible();
  ...
  test.describe('Performance', () => {
    test('should preserve sequential messages and responses', async ({ page }) => {
      // Mock API responses
      await page.route('**/api/genai/chatbot/help', async (route) => {
        const response =
          `data: {"choices":[{"delta":{"content":"Quick response"}}]}\n\ndata: [DONE]\n\n`;
        await route.fulfill({
          status: 200,
          headers: { 'content-type': 'text/event-stream' },
          body: response,
      ...
      // Send each message after the previous exchange completes
      for (let i = 1; i <= 3; i++) {
